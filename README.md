...
Hexagonal architecture (Ports & Adapters) with Domain-Driven Design: business rules live in app/Domain, framework-free, with app/Application orchestrating use cases and app/Infrastructure holding every framework-specific adapter (Eloquent, HTTP, Livewire). See ARCHITECTURE.md for the full layering convention and how to add a new module.
...
🚧 In progress. Done so far: the hexagonal architecture scaffold, the authentication baseline (Fortify, 2FA, passkeys), all database migrations (auth, RBAC, academic catalog, and the ES-01–ES-04 core tables), seeders, Eloquent models, and model factories — plus, for ES-01 through ES-04, the full Domain/Application/Infrastructure/Presentation stack: the WaiverEngine rules engine, waiver-request deduplication, the EstimatedResolutionDateAssigner (24h auto-assignment), status-transition rules, the Docencia inbox, and CRUD for waiver rules and validation precedents, all wired through DomainServiceProvider.

Still pending: email notifications on status changes, PDF/Excel export from the Docencia inbox (buttons exist, not yet connected), a program/career filter on the Docencia inbox, TypeScript, JWT-secured endpoints, the external REST API integration, and automated tests.
